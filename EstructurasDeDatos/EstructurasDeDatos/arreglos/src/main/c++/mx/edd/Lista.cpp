...
public:
    ...
    Lista<T>* mergeSort(int (*compara)(const void*, const void*)) {
        ...
        int lon = this->longitud_;
        int m = lon / 2;
        Lista<T>* lista = new Lista<T>();
        Lista<T>* l = new Lista<T>();
        Nodo<T>* n = this->cabeza_;
        int contador = 0;
        while (n != nullptr) {
            if (contador < m) {
                lista->agregaFinal(n->elemento_);
                n = n->siguiente_;
            } else {
                l->agregaFinal(n->elemento_);
                n = n->siguiente_;
            }
            contador += 1;
        }
        lista = lista->mergeSort(compara);
        l = l->mergeSort(compara);
        return merge(lista, l, compara);
    }
    ...
    static Lista<T>* mergeSort(Lista<T>* lista) {
        return lista->mergeSort(compara);
    }
    ...
    bool busquedaLineal(T elemento, int (*compara)(const void*, const void*)) {
        ...
        Lista<T>::iterator i = this->end();
        while (it != i) {
            T e = *(it);
            if (compara(&elemento, &e) == 0) {
                return true;
            }
            it++;
        }
    ...
    static bool busquedaLineal(Lista<T>* lista, T elemento) {
        return lista->busquedaLineal(elemento, compara);
    }
...
private:
     ...
     * @param lista
     * @param l
     * @return una lista ordenada que es mezcla
     * de las listas de entrada.
     */
    Lista<T>* merge(Lista<T>* lista, Lista<T>* l,
        ...
        while (nodo != nullptr && n != nullptr) {
            if (compara(nodo, n) <= 0) {
                ordenada->agregaFinal(nodo->elemento_);
                nodo = nodo->siguiente_;
            } else {
                ordenada->agregaFinal(n->elemento_);
                n = n->siguiente_;
            }
        }
        while (nodo != nullptr) {
